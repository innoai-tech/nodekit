...
const isNumberType = (schema: any): boolean => schema.type === "number" || schema.type === "integer";
...
const typeRelationKeywords: { [k: string]: Array<string> } = {
    object: [
        "properties",
...
export const intersectionOf = (...values: any[]) => {
    return Genfile.id(values.filter((v) => `${v}` !== "any").map((v) => `(${v})`).join(" & "))
}
...
const typeFromObjectJSONSchema = (f: Genfile, schema: any): any => {
    ...
    if (schema.additionalProperties) {
        const keyType = typeFromJSONSchema(f, schema.propertyNames || {type: "string"});
    ...
    if (schema.patternProperties) {
        obj["+k:string"] = unionOf(...map(values(schema.patternProperties), (s) => typeFromJSONSchema(f, s)))
    }
...
const typeFromArrayJSONSchema = (f: Genfile, schema: any): any => {
    if (isArray(schema.items)) {
        if (isObject(schema.additionalItems)) {
            return Genfile.id(`[${map(schema.items, (itemSchema) => typeFromJSONSchema(f, itemSchema)).join(", ")}]`);
        }
    ...
    if (schema.maxItems && schema.maxItems === schema.minItems) {
        return Genfile.id(`[${map(times(schema.maxItems), () => typeFromJSONSchema(f, schema.items)).join(", ")}]`);
    }
...
${map(schema.enum, (v) => `${dumpValue(Genfile.id(v))} = ${dumpValue(v)}`).join(",\n")}        
    ...
    if (schema.allOf) {
        ...
        if (extendableAllOf) {
            return Genfile.id(`extends ${map(take(schema.allOf, schema.allOf.length - 1), (s: any) => {
                return `${refID(f, s.$ref)}`
            }).join("m ")} ${typeFromJSONSchema(f, schema.allOf[schema.allOf.length - 1])}`)
        }

        return intersectionOf(...map(schema.allOf, (s) => typeFromJSONSchema(f, s)));
    }
