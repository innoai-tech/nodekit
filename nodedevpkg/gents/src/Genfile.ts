...
import {isObject, isFunction, isString, map, set, startsWith} from "@innoai-tech/lodash";
...
export class Genfile {
    ...
    async sync(file: string) {
        await writeFile(file, new Buffer([
            ...map(this._imports, (nameAndAlias, path) => {
                return `import { ${map(nameAndAlias, (alias, name) => !!alias ? `${name} as ${alias}` : name).join(", ")} } from "${path}";`;
            }),
