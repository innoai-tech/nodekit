...
import {camelCase, first, values, lowerFirst, get, set, keys} from "@innoai-tech/lodash";
...
const getRequestTypes = (f: Genfile, method: string, path: string, op: any) => {
    ...
    if (op.parameters) {
        for (const p of op.parameters) {
            ...
            if (p.in == "path") {
                hasParamInPath = true;
                requestObject.url = requestObject.url.replace(`{${p.name}}`, `\${${p.in}_${lowerCamelCase(p.name)}}`);
            }

            if (p.in == "header") {
                set(requestObject, ["headers", p.name], Genfile.id(`${p.in}_${lowerCamelCase(p.name)}`));
            }

            if (p.in == "query") {
                set(requestObject, ["params", p.name], Genfile.id(`${p.in}_${lowerCamelCase(p.name)}`));
            }
    ...
    if (op.requestBody) {
        ...
        for (const ct of contentTypes) {
            ...
            set(requestUsed, "Content-Type", Genfile.id(`${lowerCamelCase("Content-Type")}`));
            set(requestObject, ["headers", "Content-Type"], Genfile.id(`${lowerCamelCase("Content-Type")}`));
...
export const genClientFromOpenAPI = (openapi: any, requestCreator: RequestCreator): Genfile => {
    ...
    for (const path in openapi.paths) {
        ...
        for (const method in ops) {
            ...
            f.const(lowerCamelCase(op.operationId), `
