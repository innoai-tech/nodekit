...
const transformedResolve = (node: Node) => {
	return (
		node.leadingComments && node.leadingComments.find((c) => c.value === mark)
	);
};
		...
		visitor: {
			CallExpression(nodePath: NodePath<CallExpression>) {
				...
				if (
					!transformedResolve(
						nodePath.node,
					) && calleeNodePath.isMemberExpression()
				) {
					const propertyNodePath = calleeNodePath.get("property") as NodePath;
					const objectNodePath = calleeNodePath.get("object") as NodePath;

					if (
						propertyNodePath.isIdentifier() && propertyNodePath.node
							.name === "resolve"
					) {
						if (
							objectNodePath.isIdentifier() && objectNodePath.node
								.name === "require"
						) {
							...
							if (arg.isLiteral()) {
								const pair = t.arrayExpression([
									...
									t.arrowFunctionExpression(
										[],
										t.callExpression(t.identifier("import"), [arg.node]),
									),
								]);

								nodePath.replaceWith(
									t.memberExpression(pair, t.numericLiteral(0), true) as any,
								);
							}
						}
					}
