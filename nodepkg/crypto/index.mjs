...
export const generate_aes_256_key = (mode = "CFB") => {
  return init(wasmURL).then(() => _generate_aes_256_key(mode));
};

export const generate_chacha20_key = () => {
  return init(wasmURL).then(() => _generate_chacha20_key(""));
};
...
export const aes_256_decrypt = (payload, key) => {
  return init(wasmURL).then(() => _aes_256_decrypt(payload, key));
};

export const aes_256_encrypt = (payload, key) => {
  return init(wasmURL).then(() => _aes_256_encrypt(payload, key));
};

export const chacha20_encrypt = (payload, key) => {
  return init(wasmURL).then(() => _chacha20_encrypt(payload, key));
};

export const chacha20_decrypt = (payload, key) => {
  return init(wasmURL).then(() => _chacha20_decrypt(payload, key));
};
